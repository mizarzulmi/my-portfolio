...
export default function GroupedProjectSection({ projectsData }) {
  ...
  const projectsByYear = projectsData.projects.reduce((acc, project) => {
    const year = project.year || new Date(project.date).getFullYear();
    if (!acc[year]) {
      acc[year] = [];
    }
